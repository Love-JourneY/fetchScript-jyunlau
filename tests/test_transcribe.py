...
def test_transcribe_finds_new_transcript_and_copies(tmp_path: Path) -> None:
    ...
    result = Transcriber(binary=binary, transcripts_dir=transcripts).transcribe(media, copy_to=outdir)
...
def test_transcribe_error_includes_b2t_output(tmp_path: Path) -> None:
    ...
    with pytest.raises(RuntimeError) as excinfo:
        Transcriber(binary=binary, transcripts_dir=transcripts).transcribe(media)
